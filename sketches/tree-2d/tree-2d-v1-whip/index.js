...
sketch.draw( ( time, center, favoriteColor ) => {
  ...
  const end = createVector(width/2, height-margin*5)
  ...
  for (let i = 0; i < steps; i++) {
    ...
    for (let b = 0; b < branchCount; b++) {
      ...
      const wavesStrength = mappers.fn(stepsProgression, 0, 1, 0, 1, easing.easeInSine);
      const wavesCount = mappers.fn(stepsProgression, 1, 0, 0, 2, easing.easeOutQuart);
      ...
      for (let lineIndex = lineMin; lineIndex < lineMax; lineIndex += lineStep) {
        ...
        stroke(
          colors.rainbow({
            ...
            hueIndex: map(stepsProgression, 0, 1, -PI/2, PI/2)*12,
            opacityFactor
          })
        );
