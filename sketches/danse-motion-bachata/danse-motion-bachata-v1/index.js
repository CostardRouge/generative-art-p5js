...
function getTextPoints({ text, size, font, position, sampleFactor, simplifyThreshold }) {
  ...
  const textPointsCacheKey = cache.key(text, fontFamily, size, "text-points", sampleFactor)
...
sketch.draw( (time, center) => {
  ...
  const points = animation.ease({
    values: "bachata".split("").map( text => (
    // values: ["bachata", "or", "salsa"].map( text => (
      getTextPoints({
        text,
  ...
  const rotateUnit =  PI/10;

  rotateX(map(sin(time), -1, 1, -rotateUnit, rotateUnit))
  rotateY(map(cos(time), -1, 1, -rotateUnit, rotateUnit))
  rotateZ(map(sin(time), -1, 1, -rotateUnit, rotateUnit))
