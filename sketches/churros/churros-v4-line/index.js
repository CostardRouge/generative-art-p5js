...
sketch.draw((time) => {
  ...
  drawer(
    ( time, index ) => {
      ...
      const lerpMax = TAU//0+map(cos(time), -1, 1, 0.5, PI);
    ...
    ( lerpIndex, lerpMin, lerpMax, time, index ) => {
      ...
      if (options.get("change-lines-count")) {
        linesCount = map(cos(lerpIndex*5+time*1), -1, 1, 1, options.get("max-lines-count"));
      }
      ...
      for (let lineIndex = lineMin; lineIndex < lineMax; lineIndex += lineStep) {
        const vector = converters.polar.vector(
          ...
          // map(lerpIndex, lerpMin, lerpMax, 50, options.get('lines-length'), true)
          );
        push();
        ...
        stroke(
          color(
            map(sin(hueSpeed+lerpIndex*1), -1, 1, 0, 360) /
              opacityFactor,
            map(sin(hueSpeed-lerpIndex), -1, 1, 360, 0) /
              opacityFactor,
            map(sin(hueSpeed+lerpIndex), -1, 1, 360, 0) /
              opacityFactor,
              // 50
