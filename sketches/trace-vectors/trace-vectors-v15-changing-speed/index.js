...
function vl(start, end, progression, dotDrawer) {
  const final = p5.Vector.lerp(start, end, progression)

  dotDrawer?.(start)
  dotDrawer?.(end)

  line(start.x, start.y, final.x, final.y)
}
...
sketch.draw( ( time, center, favoriteColor ) => {
  ...
  const speed = animation.ease({
    values: [0.01, .01, .025, .05, .05, .075, .1, .1, .1, .05],
    duration: 1,
  ...
  gt += (
    speed
    // mappers.fn(sin(time), -1, 1, 0.01, 0.05, easing.easeInOutExpo_)
  );

  push()
  strokeWeight(1)
  const speedNormalized = map(speed, .01, .1, 0, 1);
