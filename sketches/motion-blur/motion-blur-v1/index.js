...
const alphaBlurColors = ['#FF0000', '#00FF00', '#0000FF'];
...
sketch.draw(time => {
  ...
  for (let i = 0; i < samplesPerFrame; i++) {
    const sampleTime = map(frameCount/2-1 + i * sampleOffset/samplesPerFrame, 0, totalFrames, 0, 1)%T;
    ...
    for (let colorIndex = 0; colorIndex < alphaBlurColors.length; colorIndex++) {
      ...
      fill(alphaBlurColor);
      push();
      translate(width/4*sin(TAU*colorTime*2), height/4*cos(TAU*colorTime));
