...
class Spiral {
  ...
  draw(time, index, target, lerpStep = 1 / 800) {
    ...
    for (let lerpIndex = 0; lerpIndex < 1; lerpIndex += lerpStep) {
      ...
      let f = map(lerpIndex, 0, 1, 1, 50);
      f = map(sin(time/2), -1, 1, 1, 25);
      const opacityFactor = map(
        lerpIndex,
        0,
        1,
        map(sin(lerpIndex * 10 + time + f), -1, 1, 1, 50),
        1
      );
      ...
      target.fill(
        ...
        map(cos(angle - hueCadence), -1, 1, 0, 128) / opacityFactor,
        map(sin(angle + hueCadence), -1, 1, 128, 0) / opacityFactor
      );

      let s = map(
        sin(time + waveIndex + f + map(lerpIndex, 0, 1, 0, 10) / lerpIndex),
        -1,
        1,
        10,
        70
      );
      s = sin(waveIndex + time) * 300 * cos(waveIndex + time);
      ...
      for (let i = 0; i < c; i++) {
        const x = lerp(
          lerpPosition.x + xOffset * map(sin(waveIndex+time), -1, 1, -1, 1),
