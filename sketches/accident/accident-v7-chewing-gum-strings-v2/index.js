...
class Spiral {
  ...
  draw(time, index, target) {
    ...
    const lerpStep = 1 / 700; //map(mouseY, height, 0, 1, 200, true);
    ...
    for (let lerpIndex = 0; lerpIndex < 1; lerpIndex += lerpStep) {
      ...
      let s = map(sin(time + waveIndex*2+lerpIndex), -1, 1, 10, 70);
      //sin(waveIndex + time) * 300 * cos(waveIndex + time);

      // target.translate(map(sin(waveIndex), -1, 1, -1, 1), 0);

      const a = map(sin(time+waveIndex), -1, 1, 1, 5);
      const c = map(sin(time+waveIndex), -1, 1, 1, a);
      ...
      for (let i = 0; i < c; i++) {
        ...
        target.fill(
          map(sin(angle + hueCadence + i), -1, 1, 128, 360) / 1,
          map(cos(angle - hueCadence * i), -1, 1, 0, 255) / 1,
