...
function drawGrid(columns, time) {
  ...
  grid.draw(gridOptions, (cellVector, { x, y}) => {
    const n = noise(xSign*x/columns+time, ySign*y/rows, time)*4;
...
sketch.draw( (time, center) => {
  ...
  grid.draw(gridOptions, (cellVector, { x, y}) => {
    const alpha = getAlphaFromMask({
      position: cellVector,
      maskPoints: firstLetterPoints,
      // maskPoints: points,
      // maskId: letterProgression,
      // distance: 0.025
    })
    ...
    if ( alpha ) {
      const angle = noise(cellVector.x/columns, cellVector.y/rows) * TAU;
      ...
      const d = mappers.fn(alpha, 0, 255, 0, 50, easingFunction)

      const hue = noise(
        // cellVector.x/columns,
        // cellVector.y/rows,
        rotatedY/rows/2,
        rotatedX/columns/2
        // +d/10+time/2
      ...
      point(cellVector.x, cellVector.y, d)
    }
  })
