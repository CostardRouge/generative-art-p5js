...
function drawShape( {
  ...
  for (let stepIndex = 0; stepIndex < 1; stepIndex += precision) {
    ...
    // canvas.point(position.x, position.y);
    ...
    const opacityFactor = map(
      ...
      1,
      map(
        Math.sin(stepIndex*30 + animation.sinAngle * 3),
        -1,
        1,
        1,
        1.3//*pow(lerpIndex, 1.1)
    ...
    const xOffset = size
    const yOffset = mappers.fn(
      Math.cos(animation.cosAngle*2+stepIndex*6), -1, 1, -size, size,
      easing.easeInOutSine
    );

    const thickness = 50//mappers.fn(Math.sin(animation.sinAngle+stepIndex*3), -1, 1, 60, 120, easing.easeInOutElastic);
    const branchCount = 9//mappers.fn(Math.cos(animation.sinAngle*6+stepIndex*9), -1, 1, 3, 9, easing.easeInOutSine);

    for (let branchIndex = 0; branchIndex <= branchCount; branchIndex++) {
      const branchProgression = branchIndex / branchCount;

      const x1 = animation.ease({
        values: [ position.x - xOffset, position.x + xOffset],
        currentTime: animation.time,
        easingFn: easing.easeInOutCubic
      })

      const y1 = animation.ease({
        values: [ position.y - yOffset, position.y + yOffset],
        currentTime: animation.time,
        easingFn: easing.easeInOutCubic
      })
      ...
      const a = stepIndex*stepIndex*4+branchIndex*2;
...
sketch.draw((time) => {
  ...
  drawShape( {
    ...
    end: createVector(width /2, height),
    precision: 1/80,
    size: width/1.5
  })

  image(canvases.buffer, 0, 0);
