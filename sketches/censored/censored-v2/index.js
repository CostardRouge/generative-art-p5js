  ...
  draw( index, target, lerpStep = 1 / 800) {
    ...
    for (let lerpIndex = 0; lerpIndex < 1; lerpIndex += lerpStep) {
      ...
      const xOffset = mappers.fn(
        Math.sin(animation.sinAngle*5+animation.time+animation.time+8*lerpIndex), -1, 1, -waveAmplitude, waveAmplitude,
      ...
      const s = mappers.fn(Math.sin(animation.time+8 * lerpIndex*5), -1, 1, 20, 90, easing.easeInOutQuad);
      const c = 4//mappers.fn(Math.sin(animation.sinAngle **lerpIndex), -1, 1, 2, 6, easing.easeInOutExpo);
      ...
      const a = lerpIndex*lerpIndex*32+i;
