  ...
  draw( index, target, lerpStep = 1 / 800) {
    ...
    for (let lerpIndex = 0; lerpIndex < 1; lerpIndex += lerpStep) {
      ...
      const yOffset = mappers.fn(
        Math.cos(animation.cosAngle*3+animation.time+8*lerpIndex), -1, 1, -waveAmplitude, waveAmplitude,
        easing.easeInSine
      );

      const s = mappers.fn(Math.sin(animation.time+8 * lerpIndex*lerpIndex*-5), -1, 1, 40, 90, easing.easeInOutSine);
      ...
      const a = lerpIndex*lerpIndex*16+i;
        ...
        target.circle(
          constrain(x, -width/2+90*2, width/2-90*2),
