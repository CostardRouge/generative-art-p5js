...
const drawRadialPattern = (count = 7, time) => {
  ...
  // let c = (map(cos(time), -1, 1, 15, 22));
  let c = animation.sequence("c", time, [15, 20, 25, 30]);
  ...
  iterators.angle(0, TAU, TAU / c, (angle, index) => {
    ...
    const edgeLimit = map(sin(time+index), -1, 1, 0.5, 1.5)
    ...
    iterators.vector(extendedEdge, center, p, (vector, lerpIndex) => {
      ...
      const hueIndex = 1*5+lerpIndex*10;
      ...
      // strokeWeight(mappers.fn(lerpIndex, 0, 1, 0, 70, easingFunction));
