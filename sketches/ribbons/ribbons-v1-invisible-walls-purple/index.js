...
sketch.draw((time) => {
  ...
  const a = abs(cos(time/4)); 
  
  // motionBlur( (colorTime, alphaBlurColor) => {
  //   // drawBackgroundGrid(10, colorTime, alphaBlurColor);
  //   drawBackgroundGrid(1 + (7 * a), colorTime, alphaBlurColor);
  //   drawBackgroundGrid(1 + (6.5 * a), colorTime, alphaBlurColor);
  // })

  const c = color(128, 128, 255, 255);

  drawBackgroundGrid(1 + (7 * a), time, c);
  drawBackgroundGrid(1 + (6.6 * a), time, c);

  drawer(
    ( time, index ) => {
      const lerpMin = 0;
      const lerpMax = options.get('growing-line') ? map(cos(time/2), -1, 1, 0, TAU*1.5) : TAU*1.5;
