...
function createPaths() {
  ...
  paths = Array(pathsCount)
    ...
    .map(() => {
      ...
      const start = [~~random(0, columns), ~~random(0, rows)];
      const path = [ start ];

      for (let i = 1; i < max; i++ ) {
        const XorY = random([true, false])//i % 2 === 0;
        const [ previousX, previousY ] = path[ path.length -1 ];

        for (let j = 0; j < 2; j++ ) {
          const x = XorY ? ~~random(0, columns) : previousX;
          const y = !XorY ? ~~random(0, rows) : previousY;
...
sketch.draw( (time, center) => {
  ...
  paths.forEach( (vectors, pathIndex) => {
    cross(vectors[0], 20)
    cross(vectors[vectors.length-1], 20)
    ...
    vectors.forEach( ({x, y, z}, vectorIndex) => {
      ...
      let size = mappers.circularIndex(time/4+progression, [10, 30]);
